...
void OPAssignopExp(struct ASTNode *T ){
    ...
    else
    {
        ...
        int kind;
        if(T->kind==PLUSASSIGNOP) T->kind=PLUS;
        else if(T->kind==MINUSASSIGNOP)T->kind =MINUS;
        else if(T->kind==STARASSIGNOP) T->kind=STAR;
        else if(T->kind==DIVASSIGNOP)T->kind=DIV;
        else if(T->kind==MODASSIGNOP) T->kind=MOD;
...
void AutoAddMinus(struct ASTNode *T)
{
    ...
    result.type=T->type;
    result.offset=symbolTable.symbols[T->place].offset;
    if(strcmp(T->type_id,"LPAUTOPLUS")==0||strcmp(T->type_id,"LPAUTOMINUS")==0){//左边
        opn2.kind=LPAUTO;
    }
    else{//在右边
        opn2.kind =RPAUTO;
...
void Arrayvisit(struct ASTNode* T,struct ArrayInfo* info)
{
    ...
    int diminfo[MAXDIM];
    for(int i=0;i<MAXDIM;i++) diminfo[i]=info->diminfo[i][2];
    int i=0;
