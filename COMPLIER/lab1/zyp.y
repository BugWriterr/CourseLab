...
%type   <ptr> ARRAY FORDEC FORDEC1 FORDEC2 FORDEC3 ARGSLISTS
%type  <ptr>   StructSpecifier StructName StructNameNotNUll
...
%token IF ELSE WHILE RETURN FOR SWITCH CASE COLON DEFAULT BREAK CONTINUE
%token STRUCT ARRAYLIST ARRAY_VISIT
%token STRUCTDEF STRUCTDEC STRUCT_NAME STRUCTVI VAR_STRUCT_DEF
%token FOR_DEC ARGSLIST FOR_DEC1 FOR_DEC2 FOR_DEC3
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(2,EXT_VAR_DEF,yylineno,$1,$2);;}   //该结点对应一个外部变量声明
        ...
        {
                $$=mknode(1,VAR_STRUCT_DEF,yylineno,$1);
        }
        | error SEMI   {$$=NULL;}
         ;
...
Specifier:  TYPE    
        ...
        strcpy($$->type_id,$1);
        if(strcmp($1,"int")==0) $$->type=INT;
        else if(strcmp($1,"float")==0) $$->type=FLOAT;
        else if(strcmp($1,"char")==0) $$->type=CHAR;
        else if(strcmp($1,"string")==0) $$->type=STRING;
        }
...
StructSpecifier:  STRUCT StructName LC DefList RC 
                {
                        if($2)
                        {
                                $$=mknode(2,STRUCTDEF,yylineno,$2,$4);
                                strcpy($$->type_id,$2->type_id);
                        }
                        else 
                        $$=mknode(1,STRUCTDEF,yylineno,$4);
                        
                }
                | STRUCT StructNameNotNUll DecList
                {
                        $$=mknode(2,STRUCTDEC,yylineno,$2,$3);
                }
                ;
StructName:     {$$=NULL;} 
                |ID 
                {
                        $$=mknode(0,STRUCT_NAME,yylineno);
                        strcpy($$->type_id,$1);
                }
                ;  
StructNameNotNUll:  ID {$$=mknode(0,STRUCT_NAME,yylineno);strcpy($$->type_id,$1);}
                ;   
...
ARRAY:  LB Exp RB { $$=$2;}
        |LB Exp RB ARRAY
        {
                $$=mknode(2,ARRAYLIST,yylineno,$2,$4);
        }
...
Stmt:   Exp SEMI    {$$=mknode(1,EXP_STMT,yylineno,$1);}
      ...
      ;
FORDEC: FORDEC1 FORDEC2 FORDEC3 {$$=mknode(3,FOR_DEC,yylineno,$1,$2,$3);strcpy($$->type_id,"FOR_DEC");}
        //|SEMI Exp SEMI {$$=mknode(1,FOR_DEC,yylineno,$2);}

FORDEC1: DecList SEMI{$$=mknode(1,FOR_DEC1,yylineno,$1);}
        |Specifier DecList SEMI{$$=mknode(2,FOR_DEC1,yylineno,$1,$2);}
        |SEMI {$$=NULL;}

FORDEC2: Exp SEMI{$$=mknode(1,FOR_DEC2,yylineno,$1);}
        |SEMI{$$=NULL;}

FORDEC3: Exp {$$=mknode(1,FOR_DEC3,yylineno,$1);}
        |Exp COMMA FORDEC3 {$$=mknode(2,FOR_DEC3,yylineno,$1,$3);}
        | {$$=NULL;}
...
Def:    Specifier DecList SEMI {$$=mknode(2,VAR_DEF,yylineno,$1,$2);}
        |StructSpecifier SEMI  {$$=mknode(1,VAR_STRUCT_DEF,yylineno,$1);}
        |
        ;
...
Dec:     VarDec  {$$=$1;}
       | VarDec ASSIGNOP Exp  
       {
               $$=mknode(2,ASSIGNOP,yylineno,$1,$3);
               strcpy($$->type_id,"=");
        }
        | StructSpecifier ASSIGNOP Exp
        {
                $$=mknode(2,ASSIGNOP,yylineno,$1,$3);
                strcpy($$->type_id,"=");
        }
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"=");}//$$结点type_id空置未用，正好存放运算符
      ...
      | ID LP Args RP {$$=mknode(1,FUNC_CALL,yylineno,$3);strcpy($$->type_id,$1);printf("DEBUG函数调用");}
      ...
      | ID ARRAY       {
                         $$=mknode(1,ARRAY_VISIT,yylineno,$2);
                         strcpy($$->type_id,$1);
                        }
      | Exp DOT ID     {
                                $$=mknode(1,STRUCTVI,yylineno,$1);
                                strcpy($$->type_id,$3);
                       }
//       |LC ARGSLISTS RC 
//                         {
...
ARGSLISTS:  LC Args RC COMMA  ARGSLISTS  
        {
                $$=mknode(2,ARGSLIST,yylineno,$2,$5);
                
        }
        |LC Args RC
        {
                $$=mknode(1,ARGSLIST,yylineno,$2);
        }
