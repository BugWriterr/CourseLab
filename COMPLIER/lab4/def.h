...
#define MAXLENGTH 200 //定义符号表的大小
#define MAXDIM 10     //定义数组最多的维数

#define DX 3 * sizeof(int) /*活动记录控制信息需要的单元数，这个根据实际系统调整*/
//以下语法树结点类型、三地址结点类型等定义仅供参考，实验时一定要根据自己的理解来定义

int LEV;         //层号
int flag;        //判断是否存在break或continue
int return_flag; //判断是否存在return
...
struct ASTNode
{
    ...
    union {
        ...
        char type_char[32];   //由字符生成的叶节点
        char type_string[32]; //由字符串生成的叶节点
        char struct_name[32]; //由结构生成的页结点
    };
    struct ASTNode *ptr[4];     //由kind确定有多少棵子树
    int place;                  //存放（临时）变量在符号表的位置序号
    char Etrue[15], Efalse[15]; //对布尔表达式的翻译时，真假转移目标的标号
    char Snext[15];             //结点对应语句S执行后的下一条语句位置标号
    char Sbreak[15];              //处理break跳转语句
    char Scontinue[15];             //处理continue语句
...
{                   //这里只列出了一个符号表项的部分属性，没考虑属性间的互斥
    ...
    //函数入口等实验可能会用到的属性...
    int RAutoAddnum;//又自增的个数
    struct ArrayInfo *info; //内情向量
    //接下来需要数组和结构体的相关信息
};
//符号表
struct symboltable
